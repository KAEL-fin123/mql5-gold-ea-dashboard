...
export default function SidebarAd({ position, className = '' }: SidebarAdProps) {
  ...
  const IconComponent = getAdTypeIcon(currentAd.type);
  const iconColor = getAdTypeColor(currentAd.type);
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-3">
            {React.createElement(getAdTypeIcon(currentAd.position), {
              className: `w-4 h-4 ${getAdTypeColor(currentAd.position)}`
            })}
          ...
          {/* 广告标题 */}
          <h3 className="text-base font-semibold text-foreground leading-tight mb-3 bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">
            {currentAd.title || 
