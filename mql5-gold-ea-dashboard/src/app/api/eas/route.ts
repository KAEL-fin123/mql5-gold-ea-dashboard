...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedData = data.map((item, index) => ({
      id: item.eas.id,
      name: item.eas.name,
      logo_url: item.eas.logo_url,
      description: item.eas.description,
      stats: {
...
export async function POST() {
  try {
    ...
    // 处理年份数据（去重）
    const availableYears = [...new Set(yearData?.map(item => item.year) || [])];
    ...
    monthData?.forEach(item => {
      ...
      if (!availableMonths[item.year].includes(item.month)) {
